...
filelist = dir('D:\seaclear_dataset\merged_fixed_cropped_resized\*.*g*');
filebase = 'D:\seaclear_dataset\merged_fixed_cropped_resized\';

tic
for fileIdx = 1:length(filelist)
    
    
    fullimage=imread([filebase filelist(fileIdx).name]);   
    rgbImage=double(fullimage)/255;

%     rgbImage=double(imread('2021_09_15_10_17_40_Front_fixed_twice_15-05-10.jpg'))/255;
...
%     imshow([I, fusion])
    imwrite(fusion,[filebase 'processed/' filelist(fileIdx).name]);
